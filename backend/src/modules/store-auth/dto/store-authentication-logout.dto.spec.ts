...
import { StoreAuthenticationLogoutDto } from './store-authentication-logout.dto';

describe('StoreAuthenticationLogoutDto', () => {
  ...
  it('accepts exactly one raw refresh-token string without parsing or canonicalizing it', async () => {
    ...
    expect(result).toBeInstanceOf(StoreAuthenticationLogoutDto);
    expect(result).toEqual({ refreshToken });
    expect(Object.keys(result)).toEqual(['refreshToken']);
  });

  it('leaves domain-level token format validation to the logout service', async () => {
    await expect(
  ...
  ])('rejects %s without echoing the token', async (_label, body) => {
    try {
      await validateBody(body);
      throw new Error('Expected logout DTO validation to fail');
    } catch (error) {
  ...
  async function validateBody(
    body: object,
  ): Promise<StoreAuthenticationLogoutDto> {
    return validationPipe.transform(body, {
      type: 'body',
      metatype: StoreAuthenticationLogoutDto,
    }) as Promise<StoreAuthenticationLogoutDto>;
  }
});
